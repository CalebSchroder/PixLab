...
public class Picture extends SimplePicture {
	...
	public void grayscale() {
		...
		for (Pixel[] rowArray : pixels) {
			for (Pixel pixelObj : rowArray) {
				average = (pixelObj.getRed() + pixelObj.getRed() + pixelObj.getRed())/3;
	...
	public void myCollage() {
		...
		this.copy(flower1, 0, 0);
		this.copy(gorge, 100, 100);
		this.copy(blueMark, 200, 200);
		this.mirrorVertical();
		this.write("collage.jpg");
	}
	...
	public void edgeDetection2(int edgeDist) {
		...
		for (int row = 0; row < pixels.length - 1 ; row++) {
			for (int col = 0; col < pixels[0].length - 1; col++) {
				...
				if (Math.abs(leftPixel.getAverage() - rightPixel.getAverage()) > edgeDist || Math.abs(leftPixel.getAverage() - botPixel.getAverage()) > edgeDist) {
					leftPixel.setColor(Color.BLACK);
				}else{
					leftPixel.setColor(Color.WHITE);
				}
